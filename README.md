...
spotbot is a chat bot built on the [Hubot][hubot] framework. It was
...
Then you can interact with spotbot by typing `spotbot help`.

    spotbot> spotbot help
    spotbot animate me <query> - The same thing as `image me`, except adds [snip]
    spotbot help - Displays all of the help commands that spotbot knows about.
    ...
...
available hubot plugins on [npmjs.com](npmjs.com) or by using `npm search`:
...
1. Are configurable by room (persist a threshold to the redis brain per room. For reference example see [this script's documentation](https://git.target.com/aaronkorver/spotbot/blob/master/scripts/threshold.coffee))
